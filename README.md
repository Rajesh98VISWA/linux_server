...
You can visit http://54.242.178.176/ or http://ec2-54-242-178-176.compute-1.amazonaws.com/ for the website deployed.
...
### Step 1: Start a new Ubuntu Linux server instance on Amazon EC2 
  ...
  - Check for instance IPv4 public IP - 54.242.178.176
  - we can download a .pem file and connect with following command
    ```
    ssh -i linux_server_itemcatalogue_5A7.pem ubuntu@54.242.178.176
    ``` 
...
- To check port 2200 wether working or not by `ssh -i linux_server_itemcatalogue_5A7.pem -p 2200 ubuntu@54.242.178.176` 
...
### Step 7: Create an SSH key pair for `grader`
  ...
  - Restart SSH: `sudo service ssh restart`
  - On the local machine, cheking if the grader account working or not by running this command : `ssh -i linux_server_itemcatalogue_5A7.pem -p 2200
  grader@54.242.178.176`.
...
`sudo git clone https://github.com/Rajesh98VISWA/catalog.git`.
...
- We need to change sqlite to postgresql create_engine in `__init__.py`,`db_set.py` and `state_info.py`,
   ```
...
- Click `Credentials`.
- Create an OAuth Client ID (under the Credentials tab), and add http://54.242.178.176/.xip.io and 
http://ec2-54-242-178-176.compute-1.amazonaws.com/ as authorized JavaScript 
origins.
- Add http://54.242.178.176.xip.io/login,http://54.242.178.176.xip.io/gconnect,http://54.242.178.176.xip.io/callback
...
### Step 13.2: Set up and enable a virtual host
  ...
  ```
  <VirtualHost *:80>
      ServerName 54.242.178.176.xip.io
      ServerAlias ec2-54-242-178-176.compute-1.amazonaws.com
      ServerAdmin ubuntu@ec2-54-242-178-176
...
- Open your browser to http://54.242.178.176 or http://ec2-54-242-178-176.us-east-2.compute.amazonaws.com.
